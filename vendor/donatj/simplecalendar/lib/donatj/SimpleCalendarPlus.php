...
/**
 * Simple Calendar+
 * @author Sydney Gems <sydney.gems@openmailbox.org>
 * Based from Simple Calendar
 * @author Jesse G. Donat <donatj@gmail.com>
 * @link http://donatstudios.com
 * @license http://opensource.org/licenses/mit-license.php
 *
 */
class SimpleCalendarPlus {
    ...
    private $nexEventId = 0;  // nextEvent ID
	private $now = false;
	private $daily_html = array();
    // $this->daily_html[year][day][hour][minute] give array(id, html)
	private $offset = 0;
	...
	public function addDailyHtml( $html, $start_date_string, $end_date_string = null ) {
		...
		// Add event for each day between start_date and end_date (with the same hour and same minute)
		$working_date = $start_date;
		do {
			$tDate = getdate($working_date);
			$working_date += 86400;
			$this->daily_html[$tDate['year']][$tDate['mon']][$tDate['mday']][$tDate['hours']][$tDate['minutes']] = array($this->nexEventId, $html);
		} while( $working_date < $end_date + 1 );

        $this->nexEventId++;  // define id for the next event
		...
		for( $i = 1; $i <= $no_days; $i++ ) {
			...
			if( is_array($dHtml_arr) ) {
				foreach( $dHtml_arr as $dHtml ) {
                    for ($j = 0; $j < 59; $j++) {
                        if(array_key_exists($j, $dHtml)) {
                            /*
                             * Display the event with the html value and store the ID of the event in the name element of the div element
                             */
                            $out .= '<div class="event" name="'.$dHtml[$j][0].'">' . $dHtml[$j][1] . '</div>';
                        }
                    }
                }
			}
